...
../.venv/bin/cxc bench run --suite smoke --python ../.venv/bin/python --cpython-cinderx /path/to/cinderx-python --pypy /path/to/pypy3 --nuitka /path/to/nuitka --require-cinderx-baseline --out ../data/runs --summary-out ../data/summary --ci-mode
...
Additional comparators like PyPy and Nuitka can be included with `--pypy /path/to/pypy3` and
`--nuitka /path/to/nuitka` (both can also be auto-detected from `PATH`).
